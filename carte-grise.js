/* AUTO-AB v8.87 — Carte grise dédiée + correctif entretien */
(function () {
  'use strict';
  ...
  function installMaintenanceSaveFix(){
    window.saveMaintenanceInvoice=async function(){
      ...
      try{
        ...
        const d=appData();
        if(!d)throw new Error('Données de l’application indisponibles');
        if(!Array.isArray(d.documents))d.documents=[];
        d.documents.push({id:savedId,immatriculation:im,idVehicule:im,categorie:'Entretien coût',nom:title,nomFichier:title,date,description,lienDrive:link,statut:'Actif',dateUpload:new Date().toISOString()});
        if(typeof writeDataCache==='function')writeDataCache(d);
  ...
  function forceDisplayedVersion(){const badge=document.getElementById('appVersionBadge');if(badge)badge.textContent='v8.87';document.documentElement.dataset.appVersion='8.87';}
